...
public partial struct HardTreeMovementSystem : ISystem
{
    [BurstCompile]
    public void OnUpdate(ref SystemState state)
    {
        ...
        //     }
        // }
        new HardMoveTreeJob { DeltaTime = deltaTime }.Schedule();

    }

    [BurstCompile]
    private partial struct HardMoveTreeJob : IJobEntity
    {
        public float DeltaTime;

        [BurstCompile]
        private void Execute(ref LocalTransform transform, ref AppleTreeSpeed speed, 
                                    in AppleTreeBounds bounds, ref AppleTreeSpeedVert verSpeed)
        {
            ...
            if (transform.Position.y > bounds.Up)
            {
                verSpeed.Value = -math.abs(speed.Value);
            }
            else if (transform.Position.x < bounds.Down)
            {
                verSpeed.Value = math.abs(speed.Value);
            }
